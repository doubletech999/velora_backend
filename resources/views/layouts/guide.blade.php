...
<head>
    ...
    <title>@yield('title', 'Velora Guide Panel')</title>
    ...
    <div class="flex h-screen">
        ...
        <div class="bg-blue-800 text-white w-64 min-h-screen p-4">
            <div class="flex items-center mb-8">
                <i class="fas fa-compass text-2xl mr-3"></i>
                <h1 class="text-xl font-bold">Velora Guide</h1>
            </div>
            
            <nav class="space-y-2">
                <a href="{{ route('guide.dashboard') }}" class="flex items-center p-3 rounded-lg hover:bg-blue-700 transition-colors {{ request()->routeIs('guide.dashboard') ? 'bg-blue-700' : '' }}">
                    <i class="fas fa-tachometer-alt mr-3"></i>
                    Dashboard
                </a>
                <a href="{{ route('guide.bookings') }}" class="flex items-center p-3 rounded-lg hover:bg-blue-700 transition-colors {{ request()->routeIs('guide.bookings*') ? 'bg-blue-700' : '' }}">
                    <i class="fas fa-calendar-check mr-3"></i>
                    My Bookings
                </a>
                <a href="{{ route('guide.reviews') }}" class="flex items-center p-3 rounded-lg hover:bg-blue-700 transition-colors {{ request()->routeIs('guide.reviews*') ? 'bg-blue-700' : '' }}">
                    <i class="fas fa-star mr-3"></i>
                    Reviews
                </a>
                <a href="{{ route('guide.profile') }}" class="flex items-center p-3 rounded-lg hover:bg-blue-700 transition-colors {{ request()->routeIs('guide.profile*') ? 'bg-blue-700' : '' }}">
                ...
                <!-- Logout Button -->
                <form method="POST" action="{{ route('guide.logout') }}" id="logoutForm">
                    @csrf
